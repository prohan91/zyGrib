...
CurveDrawer::CurveDrawer( GriddedPlotter *plotter, 
						  double lon, double lat, QWidget *parentWindow)
...
bool CurveDrawer::createTable( const int &iDataChosen ) 
{
	...
	for (iter=sdates.begin(), iDataCnt=0; iter!=sdates.end(); iter++, iDataCnt++) {
		...
		qvWindDev << getInboundAngle(dDirH, dDir, false) / dStep;
		dDir = dDirH;
		delete pinfo;
	}
...
void CurveDrawer::initDataPlot( const int &iDataChosen )
{
	...
	qwtCurve[2]->setPen( QPen( QBrush( QColor(132,0,132,132), Qt::SolidPattern ), 1.4, 
							  Qt::SolidLine, Qt::SquareCap, Qt::BevelJoin ) );
	qwtCurve[2]->setStyle( QwtPlotCurve::Steps );
	...
	qwtCurve[3]->setPen( QPen( QBrush( QColor(0,132,0,132), Qt::SolidPattern ), 1.4,
							  Qt::SolidLine, Qt::SquareCap, Qt::BevelJoin ) );
	qwtCurve[3]->setStyle( QwtPlotCurve::Steps );	
...
float CurveDrawer::convertData( const int &iDataKey, const float &fDataVal ) {
    ...
    switch( iDataKey ) {
        case GRB_WIND_SPEED:
		case GRB_WIND_GUST:
            fVal = Util::formatSpeed( fDataVal, false ).toFloat( &bConv );
