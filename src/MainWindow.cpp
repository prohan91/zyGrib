...
void MainWindow::InitActionsStatus ()
{
	...
	strdtc = DataCodeStr::serialize (DataCode(GRB_PRV_CUR_XY2D,LV_GND_SURF,0));
    strdtc = Util::getSetting("colorMapData", strdtc).toString();
	dtc = DataCodeStr::unserialize (strdtc);
    ...
    menuBar->setIsotherms2mStep (Util::getSetting("isotherms2mStep", 2).toInt());
    terre->setIsotherms2mStep (Util::getSetting("isotherms2mStep", 2).toInt());
    menuBar->acView_Isotherms2m->setChecked (Util::getSetting("showIsotherms2m", false).toBool());
    menuBar->acView_Isotherms2mLabels->setChecked (Util::getSetting("showIsotherms2mLabels", false).toBool());
...
mb->acMap_FindCity->setVisible(false);
    //-------------------------------------------------------
    ...
    connect(mb->acView_CurrentArrow, SIGNAL(triggered(bool)),
            this,  SLOT(slotCurrentArrows(bool)));
    ...
    connect(mb->acView_Isotherms2m, SIGNAL(triggered(bool)),
            terre,  SLOT(setDrawIsotherms2m(bool)));
    connect(mb->acView_GroupIsotherms2mStep, SIGNAL(triggered(QAction *)),
            this, SLOT(slotIsotherms2mStep()));
    connect(mb->acView_Isotherms2mLabels, SIGNAL(triggered(bool)),
            terre,  SLOT(setDrawIsotherms2mLabels(bool)));
...
// 	DBG ("open file %s", qPrintable(fileName));		
		meteoFileType = terre->loadMeteoDataFile (fileName, zoomOnGrib);
	...
	if (plotter!=NULL && plotter->isReaderOk())
	{
		...
		if (meteoFileType == DATATYPE_GRIB)
		{
			...
			menuBar->menuAltitude->setEnabled (true);
			...
			ok = plotter->hasDataType (GRB_CAPE);
			menuBar->acView_CAPEsfc->setEnabled(ok);
			
			ok = plotter->hasDataType (GRB_WAV_SIG_HT);
			menuBar->acView_SigWaveHeight->setEnabled(ok);
			ok = plotter->hasDataType (GRB_WAV_MAX_HT);
			menuBar->acView_MaxWaveHeight->setEnabled(ok);
			ok = plotter->hasDataType (GRB_WAV_WHITCAP_PROB);
			menuBar->acView_WhiteCapProb->setEnabled(ok);
			...
			ok = plotter->hasData (GRB_TEMP,LV_ABOV_GND,2);
			menuBar->acView_Isotherms2m->setEnabled(ok);
			menuBar->acView_Isotherms2mLabels->setEnabled(ok);
			menuBar->acView_GroupIsotherms2mStep->setEnabled(ok);
			menuBar->menuIsotherms2mStep->setEnabled(ok);
			
			// Set altitude menus
			bool ok9 = plotter->hasData (GRB_GEOPOT_HGT,LV_ISOBARIC,925);
			menuBar->acAlt_GeopotLine_925hpa->setEnabled (ok9);
			bool ok8 = plotter->hasData (GRB_GEOPOT_HGT,LV_ISOBARIC,850);
			menuBar->acAlt_GeopotLine_850hpa->setEnabled (ok8);
			bool ok7 = plotter->hasData (GRB_GEOPOT_HGT,LV_ISOBARIC,700);
			menuBar->acAlt_GeopotLine_700hpa->setEnabled (ok7);
			bool ok5 = plotter->hasData (GRB_GEOPOT_HGT,LV_ISOBARIC,500);
			menuBar->acAlt_GeopotLine_500hpa->setEnabled (ok5);
			bool ok3 = plotter->hasData (GRB_GEOPOT_HGT,LV_ISOBARIC,300);
			menuBar->acAlt_GeopotLine_300hpa->setEnabled (ok3);
			bool ok2 = plotter->hasData (GRB_GEOPOT_HGT,LV_ISOBARIC,200);
			menuBar->acAlt_GeopotLine_200hpa->setEnabled (ok2);
			menuBar->menuGeopotStep->setEnabled (ok9 || ok8 || ok7 || ok5 || ok3 || ok2);
			menuBar->acAlt_GeopotLabels->setEnabled (ok9 || ok8 || ok7 || ok5 || ok3 || ok2);
		...
		else if (meteoFileType == DATATYPE_MBLUE) 
		{
			...
			menuBar->updateListeDates (plotter->getListDates(),
									   plotter->getCurrentDate() );
			menuBar->menuGrib->setEnabled (true);
			menuBar->menuAltitude->setEnabled (false);
			ok = true;
			menuBar->acView_TempColors->setEnabled(ok);
			menuBar->acView_TemperatureLabels->setEnabled(ok);
			menuBar->acView_RainColors->setEnabled(ok);
			...
			menuBar->acView_DuplicateMissingWaveRecords->setEnabled(ok);
			menuBar->acView_SigWaveHeight->setEnabled(ok);
			menuBar->acView_MaxWaveHeight->setEnabled(ok);
			menuBar->acView_WhiteCapProb->setEnabled(ok);
		}
		...
		//------------------------------------------------------
		// Common actions after the load of a gridded data file
		//------------------------------------------------------
...
//-------------------------------------------------
void MainWindow::slotIsotherms2mStep()
{
    int s = 2;
    MenuBar  *mb = menuBar;
    QAction *act = mb->acView_GroupIsotherms2mStep->checkedAction();
    if (act == mb->acView_Isotherms2mStep1)
        s = 1;
    else if (act == mb->acView_Isotherms2mStep2)
        s = 2;
    else if (act == mb->acView_Isotherms2mStep5)
        s = 5;
    else if (act == mb->acView_Isotherms2mStep10)
        s = 10;
    terre->setIsotherms2mStep(s);
}
...
//-------------------------------------------------
void MainWindow::slotCurrentArrows(bool b)
{
    menuBar->acView_CurrentArrow->setEnabled(b);
}
//-------------------------------------------------
...
//-------------------------------------------------
void MainWindow::setMenubarColorMapData (const DataCode &dtc)
{
    MenuBar  *mb = menuBar;
    QAction  *act = NULL;
	switch (dtc.dataType)
...
void MainWindow::slot_GroupColorMap (QAction *act)
{
	...
	{	// search "prefered" altitude with wind data 
...
//-------------------------------------------------
void MainWindow::setMenubarAltitudeData (const DataCode &dtc)
{
    if (! terre->getGriddedPlotter() || ! terre->getGriddedPlotter()->isReaderOk())    {
		return;
    }
	...
	//DBG("%s", qPrintable(DataCodeStr::toString(dtc)) ); 
	//DBG("***");
	for (it=setalt.begin(); it!=setalt.end(); it++) {
		Altitude alt = *it;
		//DBG("%s", qPrintable(AltitudeStr::toString(alt)) );
