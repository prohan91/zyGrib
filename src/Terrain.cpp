...
// Constructeur
//---------------------------------------------------------
Terrain::Terrain (QWidget *parent, Projection *proj_, GshhsReader *gshhsReader)
    : QWidget(parent)
{
...
{
    if (drawer && drawer->getColorMapData() != dtc)
    {
...
//-------------------------------------------------------
void Terrain::setDrawIsotherms2m (bool b) {
    if (drawer->showIsotherms2m != b) {
        drawer->showIsotherms2m = b;
        Util::setSetting("showIsotherms2m", b);
        mustRedraw = true;
        update();
    }
}
//-------------------------------------------------------
void Terrain::setIsotherms2mStep (double step)
{
    if (drawer->isotherms2mStep != step) {
		Util::setSetting("isotherms2mStep", step);
		drawer->isotherms2mStep = step;
		mustRedraw = true;
		update();
    }
}
//-------------------------------------------------------
void Terrain::setDrawIsotherms2mLabels (bool b) {
    if (drawer->showIsotherms2mLabels != b) {
        drawer->showIsotherms2mLabels = b;
        Util::setSetting("showIsotherms2mLabels", b);
        mustRedraw = true;
        update();
    }
}
...
//printf("wheelEvent\n");
    ...
    timerZoomWheel->start(150); // update() seulement après une petite inactivité
}
//---------------------------------------------------------
...
//printf("release\n");
    double x0, y0, x1, y1;
/*proj->screen2map(e->x(), e->y(), &x0,&y0);
printf("clic: i=%d j=%d   x=%f y=%f\n", e->x(), e->y(), x0, y0);*/
...
/*        selX1 = ceil (selX1*12)/12;
        ...
        if (getSelectedRectangle(&x0,&y0, &x1,&y1))
        {
            ...
            // Zoom sur la sélection si Control appuyé
