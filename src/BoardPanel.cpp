						...
						const DataPointInfo &pf, const Altitude &windAlt)
{
	...
	if (cellAltitude->isVisible()) {
		if (pf.hasAltitudeData) {
			...
			for (int i=0; i<6; i++)
			{
				int P = GEOPOThgt(i);	// 925 850 700 500 300 200
				shpa.sprintf("%3d", P);
...
void BoardPanel::createMenuPopup ()
{
	...
	addAction (tr("CAPE"), "boardPanelCAPE", cellCAPE);
...
void BoardPanel::createInterface ()
{
	...
	cellCAPE = new BoardPanelCell (mainFrame, mainFrameLay);
